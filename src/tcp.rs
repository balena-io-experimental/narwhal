...
impl HttpStream for TcpStream {
    fn connect(client: Client) -> Result<TcpStream> {
        ...
        let tcp_opts = client.tcp_options.chain_err(
            || "TCP backend chosen with no TCP information",
        )?;
        ...
        Ok(TcpStream { stream: tcp_stream })
    }
    ...
    fn request(&mut self, req: http::Request) -> Result<http::Response> {
        ...
        let data = read_from_stream(&mut stream).chain_err(
            || "Could not read from tcp stream",
        )?;

        let response = http::parse_response(&data).chain_err(
            || "Could not parse engine HTTP response",
        )?;
